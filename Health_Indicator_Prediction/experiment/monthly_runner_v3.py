...
#        stats = sequential_pull(pulls)
        ...
        for i in range(len(self.results)):
            if i != len(self.results) - 1:
                if stats_pd.closed_at.dtype != 'datetime64[ns]':
                    closedAtStr = stats_pd.closed_at.apply(str).str.split(".").str[0]
                    stats_pd.closed_at = pd.to_datetime(closedAtStr, format="%Y-%m-%d %H:%M:%S")
                open_mask = (stats_pd.created_at >= self.results.dates[i]) & (
                    stats_pd.created_at < self.results.dates[i + 1]
                )
                closed_mask = (
